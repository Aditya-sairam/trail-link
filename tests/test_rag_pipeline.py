...
def check_rollback(current_success_rate: float, current_avg_matched: float):
    ...
    DEGRADATION_TOLERANCE  = 0.2   # 20% tolerance to account for guardrail variability
    ...
    with mlflow.start_run(run_name=f"rag_pipeline_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}"):
        ...
        for i, patient in enumerate(TEST_PATIENTS):
            ...
            try:
                ...
                guardrail_status = guardrail_info.get("status", "none")
                ...
                mlflow.log_metrics({
                    ...
                    f"{patient_id}_guardrail_blocked"   : 1 if guardrail_status == "blocked" else 0,
                    f"{patient_id}_guardrail_flagged"   : 1 if guardrail_status == "flagged" else 0,
                })
        ...
        mlflow.log_metrics({
            ...
            "total_guardrail_blocked" : sum(1 for r in all_results if r.get("guardrail", {}).get("status") == "blocked"),
            "total_guardrail_flagged" : sum(1 for r in all_results if r.get("guardrail", {}).get("status") == "flagged"),
        })

...
def generate_stats(df: pd.DataFrame) -> Dict[str, Any]:
    """Generate summary statistics. Separate from anomaly detection."""
    stats: Dict[str, Any] = {
        "rows":           int(len(df)),
...
def detect_anomalies(df: pd.DataFrame) -> Dict[str, Any]:
    """Detect data anomalies. Separate from stats generation."""
    ...
    for col in df.columns:
        if float(df[col].isna().mean()) >= 0.70:
            anomalies["high_missing_columns"].append({
                "col": col, "missing_pct": round(float(df[col].isna().mean()) * 100, 1)
            })
    ...
    if "Enrollment" in df.columns:
        ...
        if len(enrollment) > 0:
            ...
            iqr    = q3 - q1
            outliers = enrollment[(enrollment < q1 - 1.5*iqr) | (enrollment > q3 + 1.5*iqr)]
            if len(outliers) > 0:
...
def anomalies_found(anomalies: Dict[str, Any]) -> bool:
    """Returns True if critical anomalies exist. Used by ShortCircuitOperator."""
    return bool(
...
def run_quality_checks(
    ...
    """Clean text, run stats + anomaly detection, save reports."""
    ...
    if anomalies_found(anomalies):
        log.warning(f"⚠️ Anomalies detected")
    else:

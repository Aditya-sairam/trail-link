...
Single DAG for any condition. Pass condition via params.
  ...
  airflow dags trigger clinical_trials_pipeline --conf '{"condition": "diabetes"}'
  airflow dags trigger clinical_trials_pipeline --conf '{"condition": "breast_cancer"}'
"""
...
from src.pipelines.ingest              import download_raw_trials_csv, enrich_trials_csv
from src.pipelines.quality             import run_quality_checks, anomalies_found
from src.pipelines.validate            import run_validation
from src.pipelines.stats               import compute_stats
from src.pipelines.bias                import generate_bias_report, save_bias_report
from src.pipelines.conditions.registry import REGISTRY
...
def get_config(context: dict) -> dict:
    condition = context["dag_run"].conf.get("condition", "diabetes")
